<template>
 <v-main class="list">
    ...
    <v-card>
        ...
        <v-data-table :headers="headers" :items="todos" :search="search">
            ...
            <template v-slot:[`item.checked`]="{ item }">
                <input type="checkbox" id="checkbox" v-model="item.checked" @click="addChecked(item)">
            </template>
        </v-data-table>
    ...
    <v-card style="margin-top: 30px" v-show="listChecked.length!=0">
        <v-container>
            <ul v-for="(item, index) in listChecked" :key="index">
                <li>{{ item.task }}</li>
            </ul>
...
methods: {
    ...
    addChecked(item){
        ...
        }else this.listChecked.splice(this.listChecked.indexOf(item), 1);
    },
    deleteSelected(){
        var i=0;
        while(i < this.listChecked.length){
            this.ftodos.push(this.listChecked[i]);
            this.todos.splice(this.todos.indexOf(this.listChecked[i]), 1);
            i++;
        }
        this.listChecked=[];
     },
